...
lines = [line.rstrip('\n').split(']') for line in open('input_4.txt')]
...
for record in sortedLines:
    if isinstance(record[1], bool) == False:
        ...
        if currentGuardId not in guards:
            guards[currentGuardId] = (0,[0]*60) 
    elif record[1] == False:
        feltAsleepAt = record[0].minute
    elif record[1] == True:
        awakenAt = record[0].minute
        newTime = guards[currentGuardId][0] + awakenAt - feltAsleepAt
        newMinutes = increaseTimesInArr(feltAsleepAt,awakenAt, guards[currentGuardId][1])
        guards[currentGuardId] = (newTime, newMinutes)

maxGuardId = max(guards, key=guards.get)
(maxGuardSleptTime, maxGuardsMinutes)= guards[maxGuardId]
maxMinutesSlept = max(maxGuardsMinutes)
minuteNo = maxGuardsMinutes.index(maxMinutesSlept)
